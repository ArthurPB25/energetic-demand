print("\nEDIÇÃO E REMOÇÃO DE EQUIPAMENTO\n")
...
while permicao_edicao == 1:
    ...
    if edicao == "s":
        ...
        qntd_equipamento = int(input("\nDigite quantos tipos de equipamento a(s) casa(s) terá(ão): "))

        listagem_equipamento = []

        for i in range(qntd_equipamento):
            equipamento = input(f"\nDigite o {i + 1}° equipamento: ")
            potencia = int(input("Digite a potência em W do equipamento: "))
            uso_diario = int(input("Digite o uso diário em horas do equipamento: "))
            listagem_equipamento.append(equipamento)
